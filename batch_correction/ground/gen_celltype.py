...
adata = pg.read_input(data_location)
...
lgbm = LGBMClassifier(n_jobs = 8, metric = 'multi_error', importance_type = 'gain')
lgbm.fit(X_train, y_train, eval_set = [(X_train, y_train), (X_test, y_test)], early_stopping_rounds = 1)

idx_unknown = np.isin(adata.obs['louvain_labels'], '10')
res = lgbm.predict(adata.X[idx_unknown])
...
pg.write_output(adata, "ground_cell_types")

df_celltype = adata.obs[['cell_types', 'louvain_labels']]
...
if os.system("pegasus plot scatter --basis umap --attributes louvain_labels,cell_types {src} {outname}.celltype.umap.pdf".format(src = "ground_cell_types.h5ad", outname = os.path.splitext(data_location)[0])):
	sys.exit(1)